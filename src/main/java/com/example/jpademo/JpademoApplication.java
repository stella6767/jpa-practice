 ...
 *
 *
 * JPA로 엔티티매니저 2개 만들어서, db 2개 관리하는 법도 알아보자..
 * 더티 체킹(변경 감지)
 * JPA는 엔티티를 영속성 컨텍스트에 보관할때, 최초 상태를 복사해서 저장하는데 이것을 스냅샷이라 한다.
 ...
 *
 * 비영속, 영속, 삭제, 준영속
 * 영속 상태의 엔티티를 영속성 컨텍스트에서 분리된 것을 준영속 상태라 한다
 * 만들 수 있는 방법
 ...
 *
 * 준영속 상태의 특징
 * 1. 거의 비영속에 가깝다. (1차 캐시, 쓰기 지연, 변경감지, 지연 로딩 등.. 다 안됨)
 * 2. 식별자 값을 가지고 있다.
 *
 ...
 *
 * 영속 엔티티의 동일성 보장
 * 영속성 컨텍스트는 엔티티의 동일성을 보장한다. (실제 인스턴스가 같다. ==을 사용해 비교한다.)
 */
